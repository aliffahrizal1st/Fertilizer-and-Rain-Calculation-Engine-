...
def analyze_fertilizer(date_input, username, estate_name, blok_name, df, peilscale, last_fertilizer, last_fertilizer_date, next_fertilizer, next_fertilizer_date):
  ...
  if current_daily_rainfall >= 60:
    ...
    print(reason)
    append_to_spreadsheet(date_input, username, estate_name, blok_name, current_daily_rainfall, 0, "", datetime.datetime(1970, 1, 1), "", datetime.datetime(1970, 1, 1), reason, "")
  ...
  if(not validate_water):
    if(not rain_factor):
      ...
      append_to_spreadsheet(date_input, username, estate_name, blok_name, current_daily_rainfall, peilscale, "", datetime.datetime(1970, 1, 1), "", datetime.datetime(1970, 1, 1), reason, "")
...
def validate_rainfall_data_exists(selected_estate):
    ...
    if not isinstance(current_time_date, datetime.datetime):
         messagebox.showerror("Internal Error", "current_time_date is not a valid datetime object.")
    ...
    if total_missing_dates > 0:
        ...
        messagebox.showerror("Data Tidak Lengkap",
                             f"Terdapat {total_missing_dates} hari data hujan yang hilang untuk estate {selected_estate} "
                             f"sejak {last_reported_str}.\n\n"
                             "Harap lengkapi data melalui menu 'Masukkan Data Hujan'")
        return False
